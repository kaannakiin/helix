...
export function DrawerTreeList({
  ...
  const treeFetchOptions = fetchOptions as TreeFetchOptions;
  ...
  const { data, isLoading, fetchNextPage, hasNextPage, isFetchingNextPage } =
    useInfiniteQuery({
      queryKey: [...queryKey, 'drawer-tree-search', debouncedSearch || ''],
  ...
  const countSelectedInSubtree = useCallback(
    (node: TreeNode): number => {
      ...
      return children.reduce((acc, child) => {
        const childSelected = tempSelectedIds.has(child.id) ? 1 : 0;
        return acc + childSelected + countSelectedInSubtree(child);
      }, 0);
    },
    [tempSelectedIds, nodeChildren]
  );
  ...
  const renderNode = useCallback(
    (node: TreeNode, depth = 0): React.ReactNode => {
      ...
      if (!nodeHasChildren) {
        const content = renderLabel(node);
        return (
          <Group
            key={node.id}
            gap="sm"
