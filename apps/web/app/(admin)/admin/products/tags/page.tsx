...
export default function TagsPage() {
  ...
  const columns = useMemo(
    () => [
      createColumn<AdminTagGroupListPrismaType>('name', {
        headerKey: 'name',
        type: 'text',
        minWidth: 200,
        valueGetter: (params) => {
      ...
      createColumn<AdminTagGroupListPrismaType>('_count.tags', {
        headerKey: 'tagsCount',
        type: 'number',
        minWidth: 110,
      }),
      ...
      getRows: async (params: IGetRowsParams) => {
        try {
          ...
          const res = await apiClient.post<
            PaginatedResponse<AdminTagGroupListPrismaType>
          >('/admin/tag-groups/query', query);
  ...
  const handleExport = useCallback(
    (format: ExportFormat) => {
      ...
      downloadExport({
        endpoint: '/admin/tag-groups/export',
        format,
